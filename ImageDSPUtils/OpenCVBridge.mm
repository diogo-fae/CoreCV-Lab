...
-(Boolean)processFinger:(int)flashOn{
    cv::Mat image_copy;
    char text[50];
    Scalar avgPixelIntensity;
    int colors[3];
    ...
    cvtColor(_image, image_copy, CV_BGRA2BGR); // get rid of alpha for processing
    avgPixelIntensity = cv::mean( image_copy );
    sprintf(text,"Avg. B: %.0f, G: %.0f, R: %.0f \n", avgPixelIntensity.val[2],avgPixelIntensity.val[1],avgPixelIntensity.val[0]);
    
//    Red:      avgPixelIntensity.val[0]
//    Green:    avgPixelIntensity.val[1]
//    Blue:     avgPixelIntensity.val[2]
    
    colors[0] = avgPixelIntensity.val[0];   // Red
    colors[1] = avgPixelIntensity.val[1];   // Green
    colors[2] = avgPixelIntensity.val[2];   // Blue
    
    std::cout << "AVG: " << text << std::endl;
    
    
    if(flashOn > 0){
        if (colors[0] >= 200){
//            PROBLEM is that right after the flashlight turns on, the values are messed up, but then stabilize
//            SOLUTION is passing an integer instead of a Boolean. That way, the first frame after the
//            flashlight turns on is skipped.
            
//            std::cout << "Fingering detected!!\n";
//            std::cout << "Flashlight OFF\n";
            cv::putText(_image, "Fingering detected!!", cv::Point(10, 250), FONT_HERSHEY_PLAIN,
                        0.75, Scalar::all(255), 1, 2);
            
            if(j < 100){
                red[j] = avgPixelIntensity[0];
                green[j] = avgPixelIntensity[1];
                blue[j] = avgPixelIntensity[2];
                j++;
            }
            if(j == 100){
                cv::putText(_image, "100 fingerings gathered!", cv::Point(10, 300), FONT_HERSHEY_PLAIN,
                            0.75, Scalar::all(255), 1, 2);
            }
            std::cout << "TRUE ON\n";
            return true;
...
//        std::cout << "Flashlight OFF\n";
        if ((colors[0] >= 60) && (colors[1] <= 15) && (colors[2] <= 15)){
//            std::cout << "Fingering detected!!\n";
            cv::putText(_image, "Fingering detected!!", cv::Point(10, 250), FONT_HERSHEY_PLAIN,
                        0.75, Scalar::all(255), 1, 2);
            std::cout << "TRUE OFF\n";
